...
const FRANCHISE_STYLES = `
  ...
  .franchise-form textarea::placeholder { color: rgba(255,255,255,0.45); }
  ...
  .franchise-form select option { color: var(--text-dark); background: #fff; }
  ...
  .franchise-form .btn:disabled { opacity: 0.65; cursor: not-allowed; }
  .franchise-form-status {
    font-size: 14px; text-align: center; line-height: 1.5; margin-top: 4px;
  }
  .franchise-form-status.success { color: #c8e6c9; }
  .franchise-form-status.error { color: #ffcdd2; }
  .franchise-honeypot {
...
const FRANCHISE_FORM_SCRIPT = `<script>
(function () {
  var form = document.getElementById('franchiseInquiryForm');
  if (!form) return;
  var statusEl = document.getElementById('franchiseFormStatus');
  var submitBtn = document.getElementById('franchiseSubmitBtn');
  ...
  form.addEventListener('submit', function (e) {
    ...
    statusEl.className = 'franchise-form-status';
    ...
    fetch('https://api.web3forms.com/submit', {
      ...
      .then(function (json) {
        if (json.success) {
          statusEl.textContent = "Thanks — we'll be in touch about franchise opportunities.";
...
const CONTACT_STYLES = `
  ...
  .contact-card .value { font-size: 15px; font-weight: 700; color: var(--green-dark); display: block; max-width: 100%; overflow-wrap: anywhere; }
...
<section class="franchise-form-section" id="guide">
  <div class="franchise-section-inner">
    <h2>Franchise Opportunity Inquiry</h2>
    <p class="lead">Tell us about yourself and the market you'd like to serve. A member of our team will be in touch.</p>
    <form class="franchise-form" id="franchiseInquiryForm" novalidate aria-label="Franchise opportunity inquiry">
      <input type="hidden" name="subject" value="New Franchise Inquiry — Natural Tick Defense" />
      <div class="franchise-honeypot" aria-hidden="true">
        <input type="checkbox" name="botcheck" tabindex="-1" autocomplete="off" />
      </div>
      <label>
        Full Name *
      ...
      <label>
        Phone Number *
        <input type="tel" name="phone" autocomplete="tel" placeholder="(555) 555-5555" required />
      </label>
      <label>
        City / State of Interest *
        <input type="text" name="city_state" autocomplete="address-level2" placeholder="City, State" required />
      </label>
      <label>
        Current Occupation / Business Background
      ...
      <label>
        Why do you want to open a Natural Tick Defense franchise? *
        <textarea name="why_franchise" placeholder="Tell us what draws you to this opportunity" required></textarea>
      </label>
      <label>
        How did you hear about us?
        <input type="text" name="referral_source" placeholder="e.g. Google, referral, social media" />
      </label>
      <button type="submit" class="btn btn-primary" id="franchiseSubmitBtn">Submit Inquiry</button>
      <p class="franchise-form-status" id="franchiseFormStatus" role="status" aria-live="polite"></p>
    </form>
  </div>
</section>
